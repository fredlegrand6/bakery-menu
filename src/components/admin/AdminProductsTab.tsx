    ...
    <div ref={setNodeRef} style={style}>
      <div className="group/row flex items-center gap-3 p-4 rounded-xl bg-white/[0.015] border-b border-gold/10 border-l-2 border-l-transparent hover:border-l-gold/70 hover:bg-gold/[0.03] transition-all duration-300">
        <div {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing">
          <GripVertical size={14} className="text-gold/30 shrink-0 hover:text-gold/70 transition-colors" />
        </div>
        {children}
      </div>
    </div>
...
export default function AdminProductsTab({ onCategoryChange }: { onCategoryChange?: () => void }) {
  const { categories, refetch, addCategory, updateCategory, deleteCategory, reorderCategories } = useAdminCategories('products');
  ...
  const handleProductDragEnd = (catId: string) => (event: DragEndEvent) => {
    ...
    reorderProducts(newOrder.map((p) => p.id)).then(refetch);
  };
  ...
  const handleSaveProduct = async (data: Parameters<typeof addProduct>[0]) => {
    try {
      if (prodModal.editing) {
        await updateProduct(prodModal.editing.id, data);
      } else {
            ...
            {isOpen && (
              ...
              >
                <div className="px-5 pb-5 space-y-2 border-t border-gold/10 pt-4">
                      ...
                      {items.map((product) => (
                        <SortableProductRow key={product.id} product={product}>
                          <div
                            ...
                            style={{
                              background:
                                'linear-gradient(160deg, rgba(212,162,76,0.18), rgba(140,106,42,0.08))',
                          ...
                          <button
                            onClick={() => updateProduct(product.id, { is_available: !product.is_available }).then(refetch)}
                            className={cn(
                  ...
                  <button
                    onClick={() => setProdModal({ open: true, editing: null, categoryId: cat.id, nextOrder: items.length })}
  ...
  return (
    <div className="space-y-3">
      ...
      <ProductModal
        open={prodModal.open}
        onClose={() => setProdModal({ open: false, editing: null, categoryId: '', nextOrder: 0 })}
        onSave={handleSaveProduct}
        editing={prodModal.editing}
        categoryId={prodModal.categoryId}
        nextOrder={prodModal.nextOrder}
      />
    </div>
  );
}
