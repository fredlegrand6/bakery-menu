              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                <div className="px-5 pb-5 space-y-2 border-t border-sage/[0.08] pt-4">
                  <div className="flex gap-2 mb-3">
                    <button onClick={() => updateCategory(cat.id, { is_active: !cat.is_active })} title={cat.is_active ? 'Hide' : 'Show'} className="w-8 h-8 flex items-center justify-center rounded-xl bg-white/5 text-sage/50 hover:bg-white/10 hover:text-cream transition-colors duration-300">
                      {cat.is_active ? <EyeOff size={14} /> : <Eye size={14} />}
                    </button>
                    <button onClick={() => setCatModal({ open: true, editing: cat })} title="Edit category" className="w-8 h-8 flex items-center justify-center rounded-xl bg-white/5 text-sage/50 hover:bg-white/10 hover:text-cream transition-colors duration-300">
                      <Pencil size={14} />
                    </button>
                    <button onClick={() => handleDeleteCategory(cat.id)} title="Delete category" className="w-8 h-8 flex items-center justify-center rounded-xl bg-white/5 text-red-400/70 hover:bg-red-900/20 hover:text-red-400 transition-colors duration-300">
                      <Trash2 size={14} />
                    </button>
                  </div>
