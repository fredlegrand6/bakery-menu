              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                <div className="px-5 pb-5 space-y-2 border-t border-gold/10 pt-4">
                  <div className="flex gap-2 mb-3">
                    <button onClick={() => updateCategory(cat.id, { is_active: !cat.is_active })} className="w-8 h-8 flex items-center justify-center rounded-xl bg-white/[0.03] text-gold/60 ring-1 ring-gold/15 hover:bg-gold/10 hover:text-gold hover:ring-gold/40 transition-all duration-300">
                      {cat.is_active ? <EyeOff size={14} /> : <Eye size={14} />}
                    </button>
                    <button onClick={() => setCatModal({ open: true, editing: cat })} className="w-8 h-8 flex items-center justify-center rounded-xl bg-white/[0.03] text-gold/60 ring-1 ring-gold/15 hover:bg-gold/10 hover:text-gold hover:ring-gold/40 transition-all duration-300">
                      <Pencil size={14} />
                    </button>
                    <button onClick={() => handleDeleteCategory(cat.id)} className="w-8 h-8 flex items-center justify-center rounded-xl bg-white/[0.03] text-red-400/60 ring-1 ring-red-400/15 hover:bg-red-500/10 hover:text-red-300 hover:ring-red-400/40 transition-all duration-300">
                      <Trash2 size={14} />
                    </button>
                  </div>
