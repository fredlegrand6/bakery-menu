...
import { useProducts, useSections } from '@/hooks/useMenu';
  ...
  return (
    <section>
      <button
        ...
        className={cn(
          'md:hidden w-full flex items-center justify-between px-5 py-4 rounded-2xl transition-all duration-400 glass-soft',
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-[10px] uppercase tracking-[0.22em] text-gold/55 mt-1">
              {cat.bakery_products.length} item{cat.bakery_products.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
        <motion.div
          ...
          className={cn(
            'w-9 h-9 rounded-full flex items-center justify-center transition-colors duration-400',
            isOpen
          ...
          return (
            <div key={section.id}>
              <div className="px-1 pt-10 pb-2">
                <span className="text-[10px] uppercase tracking-[0.35em] text-gold/70">Section</span>
                <h2
                  className="font-display text-[26px] text-cream mt-1"
                  style={{ fontVariationSettings: '"SOFT" 30, "opsz" 144, "wght" 400' }}
                >
                  {section.name}
                </h2>
                <div className="hairline w-16 mt-2 mb-2" />
              </div>
